...
def make_train(config, viz_test_env):
    ...
    def train(rng):
        ...
        network = ActorCriticRNN(env.action_space(env.agents[0]).n, config=config) for _ in range(env.n_agents)
        rng, _rng = jax.random.split(rng)
        init_x = (
            jnp.zeros(
                (1, config["NUM_ACTORS"], config["NUM_ENVS"], env.observation_space(env.agents[0]).shape)
            ),
            jnp.zeros((1, config["NUM_ACTORS"], config["NUM_ENVS"])),
        )
        init_hstate = ScannedRNN.initialize_carry(config["NUM_ENVS"], config["GRU_HIDDEN_DIM"])
        network_params = jnp.vmap(network.init, in_axes=0)(_rng, init_hstate, init_x)
        if config["ANNEAL_LR"]:
            tx = optax.chain(
